...
- Jamba referansları doğrulandı: istenen `mamba`/`jamba` klasörleri yok; mevcut referanslar `mamba-cpt-tr`, `jamba-sft` ve `santosvbasvuru` içindeki model server'dır. Bu repo'da canonical servis adı `llm`, Docker içi adres `llm:8080`, gerçek Jamba overlay'i `compose.llm.yaml`dır. Public CoreAIgent adaptörü `/v1/generate`; F-03/F-04/F-05'in kontrollü internal çağrısı minimal `/generate` endpoint'idir. Ayrıntı için [F-07](07_jamba_inference_integration.md) ve [F-08](08_docker_and_runtime.md) okunmalıdır.
