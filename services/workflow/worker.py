...
# language they wrote in; the instruction shares that language because a Turkish
# instruction reliably produces a Turkish draft whatever the case text says.
# Only the wording is translated: the JSON keys and the allowed correspondence
# types are contract values and stay identical in every language.
CORRESPONDENCE_INSTRUCTIONS = {
    "tr": {
        ...
        "no_sources": "Kullanılabilir kaynak yoktur. used_source_refs=[] döndür; taslak yalnız idari inceleme veya iletim ifadeleri içersin.",
        "sources": "used_source_refs yalnızca retrieved_sources içindeki chunk_id değerleri olabilir.",
        "retry": " Önceki yanıt kabul edilmedi: {error}.",
        "input": "\nGirdi:\n",
    },
    "en": {
        "header": (
            "Produce an official English correspondence draft. Return exactly one JSON object; add no markdown, commentary or extra keys. "
            "Close the JSON object and stop writing after it. draft_text must be 2-4 short sentences and at most 1200 characters. "
            "Required schema: document_summary(string, at most 600 characters), recommended_correspondence_type("
            "response_letter|information_letter|referral_letter|cover_letter|other), draft_text(string, at most 6000 characters), "
            "used_source_refs(array of strings). If other is chosen, correspondence_type_detail(string, at most 200 characters) may be added. "
            "Example shape: {\"document_summary\":\"Short summary.\",\"recommended_correspondence_type\":\"information_letter\",\"draft_text\":\"Your application will be reviewed.\",\"used_source_refs\":[]}. "
        ),
        "no_sources": "No sources are available. Return used_source_refs=[]; the draft may only state that the case is under administrative review or has been forwarded.",
        "sources": "used_source_refs may only contain chunk_id values taken from retrieved_sources.",
        "retry": " The previous answer was rejected: {error}.",
        "input": "\nInput:\n",
    },
}
CORRESPONDENCE_FALLBACK_LANGUAGE = "tr"


def correspondence_language(language):
    """Pick the draft language, defaulting to the authority's own.

    "unknown" is a real intake outcome, not an error, so it resolves the same way
    an unsupported language does instead of blocking the generation.
    """
    return language if language in CORRESPONDENCE_INSTRUCTIONS else CORRESPONDENCE_FALLBACK_LANGUAGE


def _prompt(*, row, semantic_fields, sanitized_document, chunks, repair_error=None, language=None):
    """Build the bounded, PII-minimized F-04 model instruction."""
    wording = CORRESPONDENCE_INSTRUCTIONS[correspondence_language(language)]
    payload = {
    ...
    source_rule = wording["no_sources"] if not chunks else wording["sources"]
    retry_rule = wording["retry"].format(error=repair_error) if repair_error else ""
    return wording["header"] + source_rule + retry_rule + wording["input"] + json.dumps(payload, ensure_ascii=False)
...
def run_once():
    ...
    try:
        with psycopg.connect(DATABASE_URL) as db:
            row = db.execute("SELECT g.case_id,g.source_case_revision,g.request_type_id,g.department_label,g.unit_label,r.normalized_text,g.validated_fields,r.language FROM correspondence_generations g JOIN intake_records r USING(document_id) WHERE g.generation_id=%s", (job[1],)).fetchone()
